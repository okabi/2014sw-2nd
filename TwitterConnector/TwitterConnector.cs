...
namespace TwitterConnector
{
    ...
    public class TwitterConnector
    {
        ...
        /// <summary>
        /// ユーザを認証してOAuthを利用したAPI利用を出来るようにする。コンソールアプリケーションでの使用を前提にしているので、必要がある場合改変すること。
        /// </summary>
        ...
        public TwitterConnector(string screenName)
        {
            ...
            Dictionary<string, string> requestTokenDictionary = ParameterSplit(requestTokenString);

            // ユーザ認証処理
            System.Diagnostics.Process.Start("https://api.twitter.com/oauth/authorize?" + requestTokenString + "&screen_name=" + screenName);
